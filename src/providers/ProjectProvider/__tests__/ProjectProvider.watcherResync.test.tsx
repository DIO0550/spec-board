...
const taskA = Task.fromPayload(
  makeTaskPayload(taskFilePathFixture("tasks/a.md"), "A"),
);
...
test("watcher-resync-required は tasks と両 projection を同一 snapshot で反映する", async () => {
  const handlers = installCaptureListen();
  await mountLoaded();
  const projections = taskProjectionMap(1, 2, [
    taskFilePathFixture("tasks/rescanned.md"),
  ]);
  ...
  getTasksMock.mockResolvedValueOnce(
    getTasksOk(
      [
        Task.fromPayload(
          makeTaskPayload(taskFilePathFixture("tasks/rescanned.md"), "R"),
        ),
      ],
...
test("eventSeq を 1 つ飛ばした envelope で get_tasks が呼ばれ board が復旧する", async () => {
  ...
  const milestoneProjections = milestoneProjectionMap(2, 2, [
    taskFilePathFixture("tasks/recovered.md"),
    taskFilePathFixture("tasks/a.md"),
  ]);
  getTasksMock.mockResolvedValueOnce(
    getTasksOk(
      [
        Task.fromPayload(
          makeTaskPayload(taskFilePathFixture("tasks/recovered.md"), "R"),
        ),
      ],
...
test("resync 中に届いた新しい envelope は snapshot 適用後に replay される", async () => {
  const handlers = installCaptureListen();
  await mountLoaded();
  const replayProjections = taskProjectionMap(2, 2, [
    taskFilePathFixture("tasks/late.md"),
  ]);
  ...
  fire(
    handlers,
    "task-created",
    { task: makeTaskPayload(taskFilePathFixture("tasks/late.md"), "L") },
    {
  ...
  await act(async () => {
    resolveGetTasks(
      getTasksOk(
        ...
        milestoneProjectionMap(1, 1, [taskFilePathFixture("tasks/a.md")]),
      ),
    );
  });
  await flush();
...
test("resync 中に届いた古い envelope は snapshot を上書きしない", async () => {
  ...
  fire(
    handlers,
    "task-updated",
    { task: makeTaskPayload(taskFilePathFixture("tasks/a.md"), "STALE") },
    {
      eventSeq: WATCHER_SESSION_FIXTURE.eventSeq + 2,
  ...
  await act(async () => {
    resolveGetTasks(
      getTasksOk(
        [
          Task.fromPayload(
            makeTaskPayload(taskFilePathFixture("tasks/a.md"), "FRESH"),
          ),
        ],
        WATCHER_SESSION_FIXTURE.eventSeq + 2,
        WATCHER_SESSION_FIXTURE.revision + 5,
...
test("応答の session が別世代なら dispatch されない", async () => {
  ...
  getTasksMock.mockResolvedValueOnce(
    Result.ok({
      tasks: [
        Task.fromPayload(
          makeTaskPayload(taskFilePathFixture("tasks/other.md"), "O"),
        ),
      ],
      columns: OPEN_COLUMNS,
      doneColumn: "Todo",
      projections: taskProjectionMap(1, 1),
      milestoneProjections: milestoneProjectionMap(1, 1, [
...
test("resync 成功で tasks 参照が変わっても projection 再同期の get_tasks が飛ばない", async () => {
  ...
  getTasksMock.mockResolvedValueOnce(
    getTasksOk(
      [
        Task.fromPayload(
          makeTaskPayload(taskFilePathFixture("tasks/changed.md"), "C"),
        ),
      ],
      WATCHER_SESSION_FIXTURE.eventSeq + 1,
      WATCHER_SESSION_FIXTURE.revision + 1,
...
test("旧 project の resync が未解決でも、新 session の要求は塞がれない", async () => {
  ...
  const qOpenMilestoneProjections = milestoneProjectionMap(0, 1, [
    taskFilePathFixture("tasks/q-open.md"),
  ]);
  ...
  const qResyncMilestoneProjections = milestoneProjectionMap(1, 1, [
    taskFilePathFixture("tasks/q.md"),
  ]);
  getTasksMock.mockResolvedValueOnce(
    Result.ok({
      tasks: [
  ...
  await act(async () => {
    resolveFirst(
      getTasksOk(
        [
          Task.fromPayload(
            makeTaskPayload(taskFilePathFixture("tasks/old.md"), "OLD"),
          ),
        ],
        WATCHER_SESSION_FIXTURE.eventSeq + 1,
        WATCHER_SESSION_FIXTURE.revision + 1,
        taskProjectionMap(9, 9),
        milestoneProjectionMap(9, 9, [taskFilePathFixture("tasks/old.md")]),
      ),
    );
  });
  await flush();
...
test("resync in-flight のあいだに届いた変更は即時反映されず buffer される", async () => {
  ...
  fire(
    handlers,
    "task-created",
    { task: makeTaskPayload(taskFilePathFixture("tasks/inflight.md"), "I") },
    {
      eventSeq: WATCHER_SESSION_FIXTURE.eventSeq + 2,
      revision: WATCHER_SESSION_FIXTURE.revision + 9,
    },
  ...
  expect(currentTasks().map((task) => task.filePath)).toEqual([
    taskFilePathFixture("tasks/a.md"),
    taskFilePathFixture("tasks/inflight.md"),
  ]);
});
...
test("旧 project の応答が新 session の resync より先に着地しても buffer を壊さない", async () => {
  ...
  fire(
    handlers,
    "task-created",
    { task: makeTaskPayload(taskFilePathFixture("tasks/new.md"), "N") },
    {
      generation: nextGeneration,
      eventSeq: WATCHER_SESSION_FIXTURE.eventSeq + 2,
...
test("buffer 溢れ由来の 2 本目の取得中も、届いた変更は即時反映されず replay される", async () => {
  ...
  const overflowMilestoneProjections = milestoneProjectionMap(1, 2, [
    taskFilePathFixture("tasks/flood-last.md"),
    taskFilePathFixture("tasks/a.md"),
  ]);
  await act(async () => {
    resolveFirst(
  ...
  fire(
    ...
    "task-created",
    {
      task: makeTaskPayload(taskFilePathFixture("tasks/during-second.md"), "D"),
    },
    {
  ...
  expect(currentTasks().map((task) => task.filePath)).toEqual([
    taskFilePathFixture("tasks/a.md"),
    taskFilePathFixture("tasks/during-second.md"),
  ]);
});
...
test("読み取り中に mutation が commit したら、古い snapshot を採用せず取り直す", async () => {
  ...
  createTaskMock.mockResolvedValue(
    Result.ok(
      Task.fromPayload(
        makeTaskPayload(taskFilePathFixture("tasks/created.md"), "C"),
      ),
    ),
  );
  act(() => {
  ...
  const freshMilestoneProjections = milestoneProjectionMap(2, 2, [
    taskFilePathFixture("tasks/after-mutation.md"),
    taskFilePathFixture("tasks/a.md"),
  ]);
  getTasksMock.mockResolvedValue(
    getTasksOk(
      [
        Task.fromPayload(
          makeTaskPayload(taskFilePathFixture("tasks/after-mutation.md"), "M"),
        ),
      ],
      WATCHER_SESSION_FIXTURE.eventSeq + 2,
  ...
  await act(async () => {
    resolveStale(
      getTasksOk(
        [
          Task.fromPayload(
            makeTaskPayload(taskFilePathFixture("tasks/stale.md"), "S"),
          ),
        ],
        WATCHER_SESSION_FIXTURE.eventSeq + 1,
        WATCHER_SESSION_FIXTURE.revision + 1,
        taskProjectionMap(1, 1),
        milestoneProjectionMap(1, 1, [taskFilePathFixture("tasks/stale.md")]),
      ),
    );
  });
  await flush();
...
const rescannedTree: TaskForest = TaskForest.fromPayload([
  {
    ...
    children: [
      { filePath: taskFilePathFixture("tasks/rescanned.md"), children: [] },
    ],
  },
]);
