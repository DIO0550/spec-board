...
const makeTask = (overrides: Partial<TaskPayload> = {}): Task =>
  Task.fromPayload({
    ...
    reverseLinks: [],
    body: "",
    filePath: taskFilePathFixture("tasks/t1.md"),
    ...overrides,
  });
...
test("target が tasks に見つかれば selectedTaskId と announceText を返す", () => {
  ...
  const outcome = selectTaskOutcome([task], taskIdFixture("t1"));
...
test("target が tasks に無いとき null を返し setSelectedTaskId/announce の発火を防げる形", () => {
  ...
  const outcome = selectTaskOutcome([task], taskIdFixture("missing"));

  expect(outcome).toBeNull();
});

test("tasks が空でも null を返す（render と click の race で tasks が空に置換された場合）", () => {
  const outcome = selectTaskOutcome([], taskIdFixture("t1"));
...
test("同一 id が複数ある場合は最初の一致を採用する（task.id == filePath なので実質発生しないが防御挙動を固定）", () => {
  const first = makeTask({
    id: "dup",
    title: "先",
    filePath: taskFilePathFixture("tasks/dup.md"),
  });
  ...
  const outcome = selectTaskOutcome([first, second], taskIdFixture("dup"));
