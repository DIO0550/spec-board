...
import { unarchiveTask } from "@/lib/tauri";
...
test("invoke が 'unarchive_task' command に args キーで params を渡す", async () => {
  vi.mocked(invoke).mockResolvedValue({ restoredFilePath: "tasks/x.md" });
  await unarchiveTask({ filePath: "tasks/x.md" });
  expect(vi.mocked(invoke)).toHaveBeenCalledWith("unarchive_task", {
...
test("成功時は payload をそのまま Result.ok で返す", async () => {
  ...
  const result = await unarchiveTask({ filePath: "tasks/x.md" });
