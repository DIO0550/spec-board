...
test("finalizeLabels: 非空かつ重複は戻り値が既存 labels、state は変更しない", () => {
  const get = render(["a"]);
  act(() => {
    get().dispatch({ type: "setInput", value: "a" });
  });
  ...
  expect(result).toEqual(["a"]);
  // submit 用スナップショットのみが目的のため labelInput は保持される
  expect(get().state).toEqual({ labels: ["a"], labelInput: "a" });
});

test("finalizeLabels: 非空かつ新規は pending を取り込んだ配列を返すが state は変更しない", () => {
  const get = render(["a"]);
  act(() => {
  ...
  // submit 成功で unmount、失敗時は UI をそのまま維持するため state は不変
  expect(get().state).toEqual({ labels: ["a"], labelInput: "b" });
});
