...
test("set dispatch で labels が全置換され labelInput がクリアされる", () => {
  const get = render(["a"]);
  act(() => {
    get().dispatch({ type: "setInput", value: "typing" });
  });
  act(() => {
    get().dispatch({ type: "set", labels: ["x", "y"] });
  });
  expect(get().state).toEqual({ labels: ["x", "y"], labelInput: "" });
});
