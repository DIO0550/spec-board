...
import { buildPreviewFrontmatter, PreviewFrontmatter } from "..";
...
test("title+status のみのとき最小 frontmatter を返す", () => {
  expect(buildPreviewFrontmatter(PreviewFrontmatter.from(baseInput))).toBe(
    "---\ntitle: タスク\nstatus: todo\n---",
  );
});
...
test.each([[undefined], [""]])("due が %j のとき due 行を省略する", (due) => {
  expect(
    buildPreviewFrontmatter(PreviewFrontmatter.from({ ...baseInput, due })),
  ).toBe("---\ntitle: タスク\nstatus: todo\n---");
});
...
test.each([
  ...
  expect(
    buildPreviewFrontmatter(PreviewFrontmatter.from({ ...baseInput, draft })),
  ).toBe("---\ntitle: タスク\nstatus: todo\n---");
});
