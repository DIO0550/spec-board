...
test.each([
  ...
  "buildUniqueFileName golden: 既存 %j では連番の最初の空きを使う",
  (existing, expected) => {
...
test("compute: 親タスク指定で targetDir が親の dirname になる", () => {
  ...
  expect(result).toMatchObject({ kind: "path", relPath: "tasks/epic/child.md" });
});
...
test.each([
  ...
  "compute: parent filePath の表記揺れ %s でも targetDir が tasks/epic になる",
  (parentFilePath) => {
    const result = SavePathPreview.compute(
...
test.each([["custom.md"], ["custom.MD"]] as const)(
  "compute: 明示 fileName %s は二重拡張子にならず base 正規化が toParam と一致する",
  (fileName) => {
...
test.each([[".md"], [" .md"]] as const)(
  "compute: fileName %s（base が空になる入力）+ title ありは title 由来へフォールバックする（toParam が undefined を返す実送信パラメータと同値）",
  (fileName) => {
    const result = SavePathPreview.compute(
...
test.each([[".md"], [" .md"]] as const)(
  "compute: fileName %s + title も空なら pending を返す（invalid にしない）",
  (fileName) => {
...
test.each([["a/b"], ["a\\b"]] as const)(
  "compute: セパレータ含み fileName %s は invalid + 該当文字を返す（BE from_explicit の拒否と整合）",
  (fileName) => {
    const result = SavePathPreview.compute(computeInput({ fileName }));
...
test.each([["a:b"], ["a?b"], ["a*b"]] as const)(
  "compute: OS 予約文字含み fileName %s は invalid を返す（FileNameField.validate の error をそのまま保持）",
  (fileName) => {
    const result = SavePathPreview.compute(computeInput({ fileName }));
    expect(result.kind).toBe("invalid");
  },
...
test.each([["C:\\repo"], ["C:\\repo\\"]] as const)(
  "compute: Windows パスの projectPath %s では relPath を \\ 区切りへ変換し混在なしで結合する",
  (projectPath) => {
    const result = SavePathPreview.compute(
...
test("compute: 同じ入力での再計算は同じ結果を返す（べき等性）", () => {
  ...
  expect(SavePathPreview.compute(input)).toEqual(SavePathPreview.compute(input));
});
