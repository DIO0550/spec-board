...
test("openProject invoke 失敗 (loaded 起点) → state は error、onError 発火", async () => {
  const onError = vi.fn();
  const probe = renderHook({ onError });
  await openLoaded(probe);
...
test("createTask: 再 open 開始時に pending createTask は version mismatch で破棄される", async () => {
  const onError = vi.fn();
  const probe = renderHook({ onError });
  await openLoaded(probe);
  ...
  // 同じ path で再 open: 旧 board は即 hidden になり、pending createTask は stale 扱い
  openDirectoryDialogMock.mockResolvedValueOnce(Result.ok("/p"));
  openProjectMock.mockResolvedValueOnce(Result.ok(payload));
  let openPending!: Promise<void>;
  act(() => {
  ...
  // createTask を resolve すると queue が進み、createTask は破棄 → 再 open 実行
  let result!: Awaited<ReturnType<UseProjectResult["createTask"]>>;
  await act(async () => {
    resolveCreate(Result.ok({ ...taskB }));
    result = await pending;
