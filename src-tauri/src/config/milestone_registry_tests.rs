...
fn load_parses_definitions_with_all_fields() {
    ...
    let registry = milestone_registry_store(tmp.path())
        .load()
        .expect("load ok");
    assert_eq!(registry.milestones.len(), 1);
    let m = &registry.milestones[0];
...
fn load_ignores_unknown_keys() {
    ...
    let registry = milestone_registry_store(tmp.path())
        ...
        .expect("load ok");
    assert_eq!(registry.milestones.len(), 1);
    assert_eq!(registry.milestones[0].name, "v0.3");
}
...
fn load_empty_variants_normalize_to_default() {
    ...
    for (content, desc) in cases {
        ...
        assert!(registry.milestones.is_empty(), "{desc}: 空 Vec を期待");
    }
}
...
fn load_order_zero_is_valid() {
    ...
    let registry = milestone_registry_store(tmp.path())
        .load()
        .expect("load ok");
    assert_eq!(registry.milestones[0].order, Some(0));
}
...
fn load_invalid_order_values_fall_back_to_none() {
    ...
    for (content, desc) in cases {
        ...
        assert!(
            registry.milestones[0].order.is_none(),
            "{desc}: order は None"
        );
    }
}
...
fn plan_create_sets_updated_and_pushes() {
    ...
    let next = registry
        ...
        .expect("create ok");
    assert_eq!(next.milestones.len(), 1);
    assert_eq!(next.milestones[0].name, "v0.3");
    assert_eq!(next.milestones[0].updated.as_deref(), Some(FIXED_NOW));
...
fn plan_delete_removes_target_keeps_others() {
    ...
    let names: Vec<&str> = next.milestones.iter().map(|m| m.name.as_str()).collect();
...
fn save_rejects_inconsistent_registry() {
    let tmp = TempDir::new().unwrap();
    let registry = MilestoneRegistry {
        milestones: vec![definition("dup"), definition("dup")],
    };
    let err = milestone_registry_store(tmp.path())
        .save(&registry)
        .unwrap_err();
    assert!(
        matches!(
            err,
            SaveMilestonesError::Validation(
                MilestoneValidationError::DuplicateMilestoneName { .. }
            )
        ),
        "got {err:?}"
    );
    assert!(!tmp.path().join(".spec-board/milestones.yml").exists());
}
...
fn store_can_be_mocked_via_trait_object() {
    let registry = MilestoneRegistry {
        milestones: vec![definition("mocked")],
    };
    let store = MockStore {
