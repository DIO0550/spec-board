...
#[test]
fn upsert_of_a_differently_spelled_known_path_is_reported_as_task_updated() {
    let dir = TempDir::new().expect("tempdir");
    let (state, ctx, log) = build_installed_ctx(dir.path());
    let abs = write_md(dir.path(), "tasks/a.md", &task_md("A"));
    handle_change(&TaskFileChange::Upserted(abs), &ctx).expect("seed create");
    drain(&log);

    // 同じファイルを `tasks/./a.md` という表記揺れの path で通知する。
    // canonical identity は同じなので Created ではなく Updated として扱われる。
    write_md(dir.path(), "tasks/a.md", &task_md("A2"));
    let spelled_differently = dir.path().join("tasks").join(".").join("a.md");
    handle_change(&TaskFileChange::Upserted(spelled_differently), &ctx).expect("handler ok");
