...
- 書き込みパスセットは登録期限付きの path map として管理し、`Mutex` で排他制御
- セット登録から5秒以内にイベントが来なかった場合、次回 registry API 呼び出し時に期限切れ entry を除去する（リーク防止）
...
| 内部状態 | `Mutex<HashMap<PathBuf, Instant>>` で登録済みパスと期限を保持する |
...
| 解除責務 | 通常は `consume` / `unregister` による明示的な解除を呼び出し側が行う。イベント未到達時の stale entry は registry API 呼び出し時の期限切れ purge で除去する |
| タイムアウト | 登録期限は5秒。registry は background worker を持たず、`register` / `should_ignore` / `consume` / `unregister` / `len` 実行時に期限切れ entry を除去する |
...
| エラー | public API では Mutex が poison された場合に `Err(WriteIgnoreError::LockPoisoned)` を返す。`CleanupWorkerSpawnFailed` は互換性維持のため enum variant として残すが、現在の opportunistic cleanup 実装では返さない |
| Tauri 依存 | なし。Tauri state やファイル監視コンポーネントへの保持・統合は呼び出し側で行う |

`WriteIgnoreRegistry` は自己書き込み抑制用の registry を担当する。ファイル監視イベントを無視する判定では race-free な `consume` を使う。対応する監視イベントが届かない場合でも、次回 registry API 呼び出し時に5秒を過ぎた entry は除去される。
