...
- **全画面詳細（`detail`）への遷移**: タスクカードをクリックするとまず右スライドパネル（DetailPanel）が開く。パネル右上の「全画面で開く（⤢）」を押すと、`<main>` を占有する全画面 2 ペイン詳細ビュー（DetailScreen）へ展開する（左ペイン＝タイトル＋本文、右ペイン＝プロパティサイドバー）。
