...
- **カレンダー**: `due` 日付を月 42 cell grid または週 7 cell grid に配置し、前後の月 / 週への移動、今日への復帰、月 / 週切替を提供する。status filter は App の `columns[].order` 順、完了・期限超過判定は `doneColumn` を使う。右 sidebar に今日、今後 21 日（期限超過の未完了を含む）、status filter、期限なし / 不正期限を表示する。日 cell は最大 3 件を表示し、超過分は「ほか N 件」に集約する。task click は共通 `onTaskClick` を呼び全画面詳細へ遷移する。
  - status の配色は他ビューと同じく `columns[].color` から解決する。色未設定のカラムは status 表示順のインデックスに対応する既定パレット（light / dark 追従）になる。**設定済みカラムに存在する status** は、ボード / 一覧 / ツリー / ロードマップと同色になる。カラム名が英語か日本語かには依存しない。設定に無い status はカレンダー独自に、設定済みカラムの後ろへ名前順で並べたインデックスの色を割り当てる。
  - 日 cell の event は左ボーダーに status の色、背景にその色の淡い塗りを使う。期限超過は赤系で塗り分け、status 色より優先する。マイルストーン付きは太字かつ左ボーダーなしで区別し、背景は status 色を保つ。
  - 凡例の「完了」は `doneColumn` の色に追従する。「期限超過」「期限当日」は期限由来の意味を持つため固定色。
...
- サブバー右側（キーワード検索とフィルタ開閉の間）の「保存済みフィルタ」ドロップダウンから、現在の絞り込み条件（キーワード / ラベル / 優先度 / ステータス / マイルストーン）に名前を付けて保存し、一覧から 1 クリックで適用できる。
- 保存はプロジェクト単位のクライアントローカル（`localStorage`: `spec-board:saved-filters`、プロジェクト絶対パスをキーにした map）。`.spec-board/` には書かず、チーム共有はスコープ外。
- 同名保存は上書き。1 プロジェクトあたり 20 件まで（上限到達時は新規追加を無視し、同名上書きは常に可）。空白のみの名前は保存しない。
- 何も絞り込んでいない状態では保存入力を無効化する。一覧の各行から個別削除できる。
- 復元時はカラム・ラベル等の選択肢に存在しない条件が含まれていても、既存の「利用可能な選択肢で間引く」機構（隠れフィルタ防止）がそのまま適用される。
- 壊れた永続化値はフィールド単位で既定値（絞り込みなし）へ倒して復元する（外観設定と同じ lenient 方針）。
