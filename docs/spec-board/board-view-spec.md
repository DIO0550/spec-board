...
- **全画面タスク作成（`create`）への遷移**: タスク作成は**全画面 2 ペイン作成ビュー（TaskCreateScreen）**で行う（旧 480px モーダルは廃止）。左ペイン＝入力フォーム、右ペイン＝ライブプレビュー（frontmatter＋本文の最終形を Raw / レンダリングでトグル表示、既定はレンダリング）。`<main>` を占有し、board と二重描画されない。左ペインの入力フォームは次の入力 UI を持つ:
  - **ステータス / 優先度のチップ選択**: `<select>` ではなく色付きチップの radiogroup（クリックまたは矢印キーで選択、端は反対側へ循環）。ステータスのチップ色はボードのカラム色帯と同一の accent 解決、優先度は High=赤 / Medium=黄 / Low=青（カードの優先度バッジと同配色）+「なし」。
  ...
  - **Markdown ツールバー**: 説明欄上部に見出し / 太字 / 斜体 / 箇条書きリスト / タスクリストの 5 ボタン。選択範囲に記法を適用し（全行適用済みならトグルで剥がす）、適用後も選択範囲を復元する。
...
| 全画面作成ビュー（TaskCreateScreen）のフォーカス | `<section aria-label="タスク作成" tabIndex="-1">` のランドマークを持つ。左ペイン＝入力フォーム、右ペイン＝ライブプレビュー。⌘/Ctrl+Enter で保存、Esc / 「キャンセル」は入力ありなら破棄確認ダイアログ（`role="alertdialog"` + `aria-modal`）を経由して閉じ、戻り先（board / 元の detail）へ遷移する。ダイアログ表示中は画面側の Esc / ⌘+Enter リスナーを抑止して二重ハンドリングを防ぐ。送信中（`isSubmitting`）は Esc・閉じ操作・入力を抑止する。ステータス/優先度は radiogroup + radio（roving tabIndex・矢印キー移動）、ラベル入力は combobox + listbox、説明欄は `role="toolbar"` の Markdown ツールバー、パスプレビューは `aria-live="polite"` の可視ライブリージョンを持つ。DetailScreen 同様 focus trap は適用しない |
