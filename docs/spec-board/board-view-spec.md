...
```
┌────────────────────────────────────────────────────────────────────┐
│ プロジェクト名 › パス   ● 読込済 / N tasks   [ラベル管理][+ 新規][設定][開く] │
├────────────────────────────────────────────────────────────────────┤
│  (検索ボックスは置かない — 検索 / フィルタは V2 据え置き)            │
│                                                                    │
...
│  [+ カラムを追加]                                                   │
└────────────────────────────────────────────────────────────────────┘
  ※ 各カラム左端の ▌ = ステータス色の縦バー（FE 固定マッピング、Column 型は変更しない）
```
...
| コンポーネント | 種別 | 説明 | 振る舞い |
|:-------------|:-----|:-----|:---------|
| ヘッダーバー | ナビゲーション | パンくず（プロジェクト名 › パス）、同期ステータス、ボタン群（ラベル管理 / 新規タスク / 設定 / 開く）を表示 | 設定ボタンで board ⇄ settings をトグル（設定画面表示中は「ボードへ戻る」表記）。新規タスクボタンで作成モーダルを開く（初期ステータスは既存 `App.tsx` の `defaultCreateStatus`＝`order` 最小カラム名を流用。カラム文脈を持たないため固定ルールで決める。利用可能なカラムが無い場合は既存挙動どおりトースト「利用可能なステータスがありません」を表示しモーダルを開かない。未ロード時も同様に不可）。ラベル管理ボタンで設定画面へ遷移する（settings へ遷移するのみ。ラベルタブを選択状態にする tab state の追加は本仕様のスコープ外＝将来別 Issue で扱う）。「開く」ボタンは既存どおりプロジェクトディレクトリ選択（既存導線を維持。設定画面表示中に押した場合は押下時点でボードへ戻したうえで開く）。検索（フィルタ）は V2 据え置きで MVP では枠も置かない |
| 同期ステータス（ヘッダーバー内） | インジケータ | 既存 FE の `ProjectState` から **FE のみで導出**（BE / IPC 追加なし）。`loaded` のとき「読込済 / {tasks.length} tasks」、`loading`（open 中）のとき「同期中…」、`idle` / `error` のときは非表示 | データ源は `ProjectState.kind` と `tasks.length` のみ。**watcher の稼働状態（health）や実監視ファイル数は現行 FE が公開していない**ため、文言は「読込済 / N tasks」とし watcher 稼働を保証する表現（「監視中」等）は使わない（"N files" でもない）。watcher 実ファイル数や同期完了状態を表示したい場合は別途 BE / IPC 追加が必要で、その範囲は本仕様・V2 のスコープ外とする |
| カラム | コンテナ | ステータスに対応する縦列。ヘッダーにステータス名とタスク数を表示 + ヘッダー左端にステータス色の縦バーを表示 | ドロップ先として機能。カラム内でのカード並び替えも可能。縦バー色は FE 固定マッピング（ステータス名 → 色）で解決し、マッピングに無いステータス名は accent 単色にフォールバックする |
| カラムヘッダー | ヘッダー | ステータス名、タスク件数、追加ボタン、左端のステータス色縦バー | ステータス名クリックで名前編集。「+ 追加」で新規タスク作成。縦バー色は FE 固定マッピングで解決（未定義ステータスは accent 単色） |
...
各カラムヘッダー左端に、ステータスを示す色の縦バーを表示する。

- **データモデル変更なし**: `Column` 型は `{ name, order }` のまま（`color` を持たせない）。
  config.json スキーマ・BE Rust も変更しない。色は **FE 側の固定マッピング**で表現する。
- **マッピング**: ステータス名 → oklch トークン。よく使われる既知 5 ステータス（Backlog /
  Todo / In Progress / In Review / Done）に色を割り当てる。**これは色マッピングの対象名であって、
  デフォルトカラム集合を定義するものではない**。空プロジェクトのデフォルトカラム（Todo /
  In Progress / Done）や `config-spec.md` の `Config::default`（3 カラム）は変更しない。
  マッピングに含まれる Backlog / In Review は「その名前のカラムが存在すれば色が付く」既知名に過ぎない。
- **フォールバック**: 固定マッピングに無いステータス名（ユーザー定義のカスタムカラム）は
  **accent 単色**（参照デザインの `var(--accent)` フォールバックと一致）。
- **ラベル色との境界**: カラムのステータス色とラベル色（`labels` の oklch トークン）は
  別系統である。ラベル色の解決規則は [label-registry-spec.md](./label-registry-spec.md) を
  source of truth とし、本節はカラム縦バーのみを対象とする。両者は
  **解決規則・設定ファイル・変更責務を共有しない**（カラム色は FE 固定マッピング、
  ラベル色は labels.yml レジストリ）。同じ色相が偶然近くなることは許容する
  （oklch 値の一意性は要求しない）。ラベルレジストリ仕様の確定により、ラベル色の
  source of truth は label-registry-spec.md に一本化済みであり、本節のカラム色は
  それに準拠する（新規のラベル色実装は伴わない）。
- **oklch の既知事項**: 対象環境（Tauri WebView / Chromium 系）で oklch をサポートする。
  彩度の高い値は gamut mapping で sRGB に丸められうる点は label-registry-spec.md と同じ既知事項。
