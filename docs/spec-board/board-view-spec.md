...
└────────────────────────────────────────────────────────────────────┘
  ...
  ※ 検索ボックスを置かないのは IDEシェル装飾要素全体の V2 据え置きの一部（スコープは「MVP / V2 スコープ」節を参照）
```
...
- **検索・フィルタリング機能はMVPスコープ外**（ラベル、優先度、キーワードによるタスク絞り込みはV2以降で検討。IDEシェル装飾要素全体のスコープ境界は次節「MVP / V2 スコープ」を参照）

## MVP / V2 スコープ（IDEシェル要素）

参照デザインには「IDEシェル」と呼ぶ装飾フレーム要素（左サイドバー / サブバータブ /
検索・フィルタ / テーマ・密度・アクセント切替）が含まれる。本節では、これら 4 要素を
MVP で採用するか V2 据え置きとするかの判断を 1 箇所に集約する（Issue #264）。

**前提（HeaderBar 採用済みとの切り分け）**: ヘッダーバー（パンくず + 同期ステータス +
ボタン群）の刷新は **MVP で採用済み**である（[コンポーネント](#コンポーネント)節を参照）。
本節が「V2 据え置き」とするのは、ヘッダーバー**以外**の IDEシェル装飾要素 4 種である。
両者を混同しないこと。

**本節はドキュメント上のスコープ宣言であり、コード変更を一切伴わない**。下記 4 要素は
いずれも現状の実装に存在せず（サイドバー / サブバータブ / 検索 / フィルタの各コンポーネント、
およびテーマ・アクセント・密度の切替トークンは未実装）、「V2 据え置き」は現状実装と一致する。

| 要素 | 判断 | V2 据え置きの理由 | V2 で再検討する条件 |
|:-----|:-----|:------------------|:--------------------|
| 左サイドバー（プロジェクトスイッチャー / ファイルツリー） | **V2 据え置き** | MVP は単一プロジェクト前提で、現行 HeaderBar の「開く」導線で充足するため枠も置かない | 複数プロジェクト切替やファイルツリー導線が必要になった段階 |
| サブバータブ（一覧 / ツリー / カレンダー等のビュー切替） | **V2 据え置き** | MVP のビューはボード（カンバン）単一で、他ビューを持たないためタブ自体が不要 | ツリー / カレンダー等の代替ビューを追加する段階 |
| 検索 / フィルタ | **V2 据え置き** | ラベル・優先度・キーワード絞り込みは MVP スコープ外。MVP では検索ボックスの枠も置かない（既存記述を維持） | タスク件数増加で絞り込み導線が必要になった段階（[制限事項](#制限事項)・[レイアウト](#レイアウト)・[コンポーネント](#コンポーネント)の既存記述と一致） |
| テーマ / 密度 / アクセント切替 | **V2 据え置き** | MVP ではアプリ全体のテーマ / 密度 / アクセントを切り替える UI・状態・CSS トークンを持たない。ラベル色は [label-registry-spec.md](./label-registry-spec.md) の現行 light パレットのみを前提とし、これはラベル色仕様でありアプリのテーマ切替とは別系統である | dark テーマ（`DARK_PALETTE`）の追加や密度・アクセントのカスタマイズ要求が出た段階（[label-registry-spec.md](./label-registry-spec.md) の「現状は light パレットのみ。将来 DARK_PALETTE を追加」と整合） |

> 上記はすべて Issue #253（ボード画面デザイン採用方針の確定）/ #264 で確定済みの
> 「HeaderBar 刷新のみ採用・左サイドバー / サブバータブ / テーマ・密度・アクセント切替は
> V2 据え置き」を、本仕様の source of truth として正式に固定したものである。検索 / フィルタの V2 据え置きは
> 本節新設より前からレイアウト図 / コンポーネント / 制限事項に分散記載されており、本節はそれらを削除せず集約・参照する。
