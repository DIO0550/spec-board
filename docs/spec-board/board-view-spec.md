...
| コンポーネント | 種別 | 説明 | 振る舞い |
|:-------------|:-----|:-----|:---------|
| ヘッダーバー | ナビゲーション | ビュー固有アクション（マイルストーン切替 / 設定 / 開く）を右寄せで表示。**プロジェクト名見出しとテーマトグルは持たない**（サイドバーの ProjectSwitcher / ThemeToggleButton へ集約済み） | 設定ボタンで設定画面へ切り替える。設定画面表示中は同ボタンが「ボードへ戻る」になり、押すとボードへ戻る。「開く」でプロジェクトディレクトリを選択（設定画面表示中に押した場合は押下時点でボードへ戻したうえで開く）。マイルストーンボタンは `onMilestoneClick` 指定時のみ表示し、milestone ビュー中は「ボードへ戻る」表記になる |
